...
func NewDB(user, pwd, dbname, host string, port int) (*DB, error) {

	con := fmt.Sprintf("postgres://%s:%s@%s:%d/%s", user, pwd, host, port, dbname)
	pool, err := pgxpool.Connect(context.Background(), con)
	if err != nil {
